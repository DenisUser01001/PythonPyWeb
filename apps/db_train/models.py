...
class Author(models.Model):
    phone_regex = RegexValidator(regex=r'^\+79\d{9}$',
                                 message="Телефонный номер должен быть в формате: '+79123456789'")
    ...
    username = models.SlugField(verbose_name='Имя аккаунта',
                                ...
                                unique=True)
    email = models.EmailField(verbose_name='email', help_text='Введите Вашу электронную почту', unique=True)
...
class Entry(models.Model):
    ...
    def __str__(self):
        return f"Автор: {self.author.username}, Текст статьи: {self.text}"
